...
class FocalLossDecoded(nn.Module):
    ...
    def forward(self, logits_decoded: torch.Tensor, targets: torch.Tensor,
                lam: float = 1.0, targets_b: torch.Tensor = None) -> torch.Tensor:
        """
...
class MultiTaskLoss(nn.Module):
    ...
    def _task_loss(self, task: str, logits: torch.Tensor, targets: torch.Tensor,
        ...
        ord_loss = ord_fn(logits, targets, lam=lam, targets_b=targets_b)
        cls_logits = self.ordinal_to_class_logits(logits)
        foc_loss   = foc_fn(cls_logits, targets, lam=lam, targets_b=targets_b)

        total = self.alpha_ord * ord_loss + self.alpha_foc * foc_loss
        return total, {"ord": ord_loss.item(), "foc": foc_loss.item()}
    ...
    def forward(self, preds: dict, targets: dict,
        ...
        total = loss_mgi + loss_ohi + loss_gei
        task_losses = {
            "mgi": loss_mgi.item(),
            "ohi": loss_ohi.item(),
            "gei": loss_gei.item(),
...
def tta_predict(model: nn.Module,
    ...
    return {k: (v > 0.5).sum(dim=1).long() for k, v in avg.items()}
...
def build_composite_sampler(dataset, indices: list) -> WeightedRandomSampler:
    """
    Composite multi-task sampler: weight = geometric-mean of per-task
    inverse-frequency weights.  Balances ALL tasks, not just MGI.
    """
    ...
    w_mgi = inv_freq(mgi_arr, CONFIG["num_classes_mgi"])
    w_ohi = inv_freq(ohi_arr, CONFIG["num_classes_ohi"])
    w_gei = inv_freq(gei_arr, CONFIG["num_classes_gei"])

    # Geometric mean → composite weight
    composite = (w_mgi * w_ohi * w_gei) ** (1.0 / 3.0)
...
def run_epoch_train(model, loader, optimizer, loss_fn,
    ...
    for step, batch in enumerate(loader):
        ...
        with _autocast():
            preds              = model(f, l, r)
            total_loss, tl     = loss_fn(preds, targets, lam, targets_b)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for task in ("mgi", "ohi", "gei"):
                ...
                y_pred[task].extend(
                    decode_ordinal(preds[task]).cpu().numpy().tolist()
                )
...
def compute_metrics(y_true: dict, y_pred: dict, class_counts: dict) -> dict:
    """Compute macro-F1 and accuracy for each task."""
    results = {}
    for task, n_cls in class_counts.items():
        labels = list(range(n_cls))
        f1  = f1_score(y_true[task], y_pred[task], labels=labels,
                       average="macro", zero_division=0)
        acc = accuracy_score(y_true[task], y_pred[task])
        results[task] = {"f1": f1, "acc": acc}
    return results
...
for fold_idx, (train_idx, val_idx) in enumerate(
    ...
    # ── DataLoaders ──────────────────────────────────────────────────────────
    sampler = build_composite_sampler(FULL_TRAIN_DS, list(train_idx))
    train_loader = DataLoader(
        Subset(FULL_TRAIN_DS, list(train_idx)),
    ...
    for epoch in range(1, CONFIG["num_epochs"] + 1):
        ...
        # ── Validation ───────────────────────────────────────────────────────
        val_loss, yt, yp = run_epoch_eval(model, val_loader, loss_fn, DEVICE)
    ...
    # ── Fold summary ──────────────────────────────────────────────────────────
    best_epoch_data = max(fold_history, key=lambda h: h["val_avg_f1"])
    ...
    print(f"\n  Fold {fold_idx} best: avg_f1={best_f1:.4f}  "
...
print(f"{'='*70}")
print(df_summary[["fold","best_val_f1","best_epoch","f1_mgi","f1_ohi","f1_gei"]].to_string(index=False))
print(f"\n  Mean ± Std across {effective_k} folds:")
for col in ["best_val_f1","f1_mgi","f1_ohi","f1_gei"]:
    v = df_summary[col]
...
def eval_with_tta(ckpt_path, val_loader):
    ...
    for batch in val_loader:
        ...
        preds = tta_predict(m, f, l, r, n_tta=CONFIG["tta_steps"])
        for task in ("mgi", "ohi", "gei"):
...
yt_best, yp_best = eval_with_tta(best_ckpt_path, val_loader_best)
yt_swa,  yp_swa  = eval_with_tta(best_swa_path,  val_loader_best)
...
for i, summary in enumerate(fold_summaries):
    final_model_paths.append(summary["swa_checkpoint"])  # prefer SWA
